...
if (isset($_POST['delete_record_no'])) {
    // 获取要删除的记录编号
    $exercise_record_no = $_POST['delete_record_no'];
    
    // 执行删除操作
    $sql = "DELETE FROM exercise WHERE exercise_record_no = $exercise_record_no";
    if ($conn->query($sql) === TRUE) {
        echo "<script>alert('数据删除成功'); window.location.href = 'exercisePage.php';</script>";
    } else {
        echo "<script>alert('数据删除失败'); window.location.href = 'exercisePage.php';</script>";
    }
}
$pet_no = $_SESSION['pet-no'];
$userId = $_SESSION['user-id'];
// 执行查询操作获取最近的 5 个常见组合
$sql = "SELECT exercise_record_no, pet_no, duration, exercise_date, exercise_type, record_time
        FROM exercise
        WHERE pet_no = $petNo
...
if ($result->num_rows > 0) {
    ...
    while ($row = $result->fetch_assoc()) {
        ...
        echo '<td><button onclick="deleteRecord(' . $row['exercise_record_no'] . ')">Delete it</button></td>';
        echo '</tr>';
    }
    echo '</tbody>';
    echo '</table>';
} else {
    echo 'No exercise record, Try to Save it!';
}
