...
<?php
        ...
        $sql1 = "SELECT count(*) from hydration";
        ...
        $start = ($page - 1) * $num;
        $sql2 = "SELECT * FROM hydration LIMIT $start, $num";
...
<body>
    ...
    <div class="content-wrapper">
        ...
        <div class="mdc-drawer__content">
            ...
            <a class="mdc-deprecated-list-item mdc-deprecated-list-item--activated" href="admin_hydration.php" aria-current="page">
                <span class="mdc-deprecated-list-item__ripple"></span>
                <i class="material-icons mdc-deprecated-list-item__graphic" aria-hidden="true">folder</i>
    ...
    <div class="main-content">
        <div class="box">
            <div><!-- Search form with dropdown menu and input field -->
                <p>Hydration</p>
            <div class="bar">
                <form action="admin_hydration_search.php" method="post">
                        <!-- Link to search records -->        
                        <select id="search_field" name="search_field" style="margin-right: 10px; height: 26px; padding: 2px;">
                            <option value="hydration_record_no">Hydration Record No</option>
                            <option value="user_id">User ID</option>
                            <option value="pet_no">Pet No</option>
                            <option value="hydration_date">Hydration Date</option>
                        </select>
    ...
    <form action="admin_hydration_multidelete.php"method="POST">
<!-- 表格主体 -->
            ...
            <div class="mdc-data-table__table-container">
                ...
                <thead>
                    ...
                    <th class="mdc-data-table__header-cell" role="columnheader" scope="col">Hydration Record No</th>
                        ...
                        <th class="mdc-data-table__header-cell" role="columnheader" scope="col">Hydration Date</th>
                ...
                if (isset($_SESSION['searchResults'])) {
                    ...
                    foreach ($subset as $row) {
                        ...
                        <tr class="mdc-data-table__row">
                            <td class="mdc-data-table__cell">{$row['hydration_record_no']}</td> 
                            <td class="mdc-data-table__cell" scope="row">{$row['user_id']}</td>
                            <td class="mdc-data-table__cell">{$row['pet_no']}</td>
                            <td class="mdc-data-table__cell">{$row['hydration_date']}</td>
                            <td class="mdc-data-table__cell">
                                <a class="material-icons"  onclick="showUpdateDialog('{$row['hydration_record_no']}')">edit</a>
                                <a class="material-icons" onclick="javascript:if(!confirm('Are you sure you want to delete the selected information?')){return false;}" href="admin_hydration_delete.php?hydration_record_no={$row['hydration_record_no']}">delete</a>
                            </td>                                                   
                            <td class="mdc-data-table__cell"><input type="checkbox" name="check[]" value="{$row['hydration_record_no']}"></td>           
                        </tr>
                A;
                        echo $html;
                    }
            ...
            <div id="dialog" class="mdc-dialog">
                <div class="mdc-dialog__container">
                    <div class="mdc-dialog__surface"
                        ...
                        <form action="admin_hydration_add.php" method="post">
                        <div class="mdc-dialog__content" id="my-dialog-content">
                            <h3>Add New Record</h3>
                                ...
                                <label for="hydration_date">Hydration Date:</label>
                                <input type="date" name="hydration_date" id="hydration_date" required><br>                     
                        </div>
...
<div id="update_dialog" class="mdc-dialog">
    <div class="mdc-dialog__container">
        <div class="mdc-dialog__surface"
            ...
            aria-labelledby="my-dialog-title"
            aria-describedby="my-dialog-content">
            <form action="admin_hydration_update.php" method="post">
                <div class="mdc-dialog__content" id="my-dialog-content">
                    <h3>Update Record</h3>
                    <label for="u_hydration_record_no">Hydration Record No:</label>
                    <input type="number" id="u_hydration_record_no" name="u_hydration_record_no" value="<?php echo $userDetails['hydration_record_no']; ?>" readonly><br>
                    <label for="u_user_id">User ID:</label>
                    <input type="number" id="u_user_id" name="u_user_id" value="<?php echo $userDetails['user_id']; ?>" required><br>
                    <label for="u_pet_no">Pet No:</label>
                    <input type="number" id="u_pet_no" name="u_pet_no" value="<?php echo $userDetails['pet_no']; ?>" required><br>
                    <label for="u_hydration_date">Hydration Date:</label>
                    <input type="date" id="u_hydration_date" name="u_hydration_date" value="<?php echo $userDetails['hydration_date']; ?>" required><br>
                </div>
                <div class="mdc-dialog__actions">
                    <button type="button" class="mdc-button mdc-dialog__button" data-mdc-dialog-action="cancel">
                        <div class="mdc-button__ripple"></div>
    ...
    <script>
        ...
        function showUpdateDialog(hydration_record_no) {
            ...
            xhr.onreadystatechange = function() {
                if (xhr.readyState === XMLHttpRequest.DONE) {
                    if (xhr.status === 200) {
                        // 请求成功
                        var hydrationDetails = JSON.parse(xhr.responseText);
                        // 在这里填充弹窗中的表单字段
                        document.getElementById("u_hydration_record_no").value = hydrationDetails.u_hydration_record_no;
                        document.getElementById("u_user_id").value = hydrationDetails.u_user_id;
                        document.getElementById("u_hydration_date").value = hydrationDetails.u_hydration_date;
                        document.getElementById("u_pet_no").value = hydrationDetails.u_pet_no;
                        // 显示弹窗
                        update_dialog.open();
                    } else {
                        // 请求失败
            ...
            // 发送AJAX请求
            xhr.open('GET', 'admin_hydration_getdet.php?hydration_record_no=' + hydration_record_no, true);
            xhr.send();
        }
