...
if (isset($_POST["Submit"])) {
    ...
    if (empty($Phone)) {
        ...
        // Insert user data
        $sql = "INSERT INTO user (phone, user_name, gender, email, password,address) VALUES ('$Phone', '$Name', '$Gender', '$Email', '$Password','$address')";
...
<form name="myForm" style="text-align: left;" action="<?php echo htmlspecialchars($_SERVER["PHP_SELF"]);?>" method="post"> 
  ...
  <p><div style="color:red" class="logo"><?php echo $error ?></div></p>   <!--set error style and print-->
   ...
   <tr>
        ...
        <td>
        <br><select class="logo" name="address">
    <option class="logo" value="Beijing" <?php if ($address == 'Beijing') echo 'selected'; ?>>Beijing</option>
    <option class="logo" value="Shanghai" <?php if ($address == 'Shanghai') echo 'selected'; ?>>Shanghai</option>
    <option class="logo" value="London" <?php if ($address == 'London') echo 'selected'; ?>>London</option>
    <option class="logo" value="Sydney" <?php if ($address == 'Sydney') echo 'selected'; ?>>Sydney</option>
</select>
</select>
        </td>
   </tr>
